...
from .test_spectrum2d import check_spectrum2d
...
def rand_spectrum2d(seed: int = 1,
    ...
    spectrum = Spectrum2D(x_data=x_bins,
                          y_data=y_bins,
                          z_data=rng.random([len(x_bins) - 1, len(y_bins) - 1]) * ureg("millibarn / meV"),
                          metadata=metadata)
    return spectrum
...
class TestSpectrum2DCollectionCreation:
    ...
    def test_init_from_spectra(self):
        ...
        spectrum = Spectrum2DCollection.from_spectra([spec_2d, spec_2d_consistent])
