...
from ..utils import get_data_path, mock_has_method_call
...
class TestUnit:
    ...
    def inject_mocks(self, mocker):
        ...
        phonon_mock = Mock()
        mocker.patch("scripts.dispersion.PhononData.from_castep", return_value=phonon_mock)
    ...
    def test_when_not_called_with_up_and_down_both_are_true(self, inject_mocks):
        scripts.dispersion.main([SharedCode.get_phonon_file()])
    ...
    def test_when_called_with_up_then_down_is_false(self, inject_mocks):
        scripts.dispersion.main([SharedCode.get_phonon_file(), "-up"])
        call_kwargs = scripts.dispersion.plot_dispersion.call_args[1]
    ...
    def test_when_called_with_down_then_up_is_false(self, inject_mocks):
        scripts.dispersion.main([SharedCode.get_phonon_file(), "-down"])
        call_kwargs = scripts.dispersion.plot_dispersion.call_args[1]
    ...
    def test_when_called_with_phonon_data_file_then_correct_data_is_loaded(self, inject_mocks):
        phonon_file = SharedCode.get_phonon_file()
        path, filename = os.path.split(phonon_file)
        scripts.dispersion.main([phonon_file])
        assert scripts.dispersion.PhononData.from_castep.call_args[0][0] == filename.split(".")[0]
        assert scripts.dispersion.PhononData.from_castep.call_args[1]["path"] == path
    ...
    def test_when_called_with_reorder_freqs_then_freqs_are_reordered(self, inject_mocks):
        scripts.dispersion.main([SharedCode.get_phonon_file(), "-reorder"])
    ...
    def test_when_called_without_grace_but_save_then_saved(self, inject_mocks):
        ...
        scripts.dispersion.main([SharedCode.get_phonon_file(), "-s={}".format(filename)])
    ...
    def test_when_called_with_grace_then_grace_and_not_saved_or_shown(self, inject_mocks):
        scripts.dispersion.main([SharedCode.get_phonon_file(), "-grace"])
    ...
    def test_when_called_with_btol_then_btol_is_used(self, inject_mocks):
        btol = 13.0
        scripts.dispersion.main([SharedCode.get_phonon_file(), "-btol={}".format(btol)])
    ...
    def test_when_called_with_mev_units_then_mev_is_used(self, inject_mocks):
        units = "meV"
        scripts.dispersion.main([SharedCode.get_phonon_file(), "-units={}".format(units)])
