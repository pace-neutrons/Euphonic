...
from utils import get_qpts, get_seednames, get_data_path,\
    get_structure_factor_data_file, get_fine_phonon_data_file,\
    get_use_c_and_n_threads, get_structure_factor_num_of_repeats,\
    get_fine_phonon_num_of_repeats
from euphonic.data.interpolation import InterpolationData
from test_benchmark_idata import get_calc_structure_factor_mean_runtime,\
    get_calc_fine_phonons_mean_runtime
...
def generate_structure_factor_data():
    ...
    for seedname in get_seednames():
        ...
        idata.calculate_fine_phonons(
            qpts, use_c=True,
            fall_back_on_python=False, n_threads=5
        )
        time = get_calc_structure_factor_mean_runtime(
            idata=idata,
            num_of_repeats=get_structure_factor_num_of_repeats()
        )
        data[seedname] = time
...
def generate_fine_phonons_data():
    ...
    data = {}
    for seedname, (use_c, n_threads_list) in product(
            get_seednames(), get_use_c_and_n_threads()
    ):
