...
class Spectrum1DCollection(collections.abc.Sequence, Spectrum):
    ...
    def to_text_file(self, filename: str,
        ...
        header = []
        header.append(f'Generated by Euphonic {__version__}')
        header.append(f'x_data in ({self.x_data.units})')
        header.append(f'y_data in ({self.y_data.units})')
        header.append(f'Common metadata: {common_metadata}')
        header.append(f'Column 1: x_data')
        for i, line in enumerate(line_data):
            header.append(
                f'Column {i + 2}: y_data[{i}] {line}'
                )
    ...
    def from_text_file(cls: Type[T], filename: str) -> T:
        ...
        return cls(data[:, 0]*ureg('dimensionless'),
                   data[:, 1:].transpose()*ureg('dimensionless'))
