...
def _read_phonon_data(path='.', phonon_name='', phonon_format=None,
    ...
    try: # load summary file
        ...
        raise Exception('Summary file {summary_pathname} not found.')
...
def _read_interpolation_data(path='.', summary_name='phonopy.yaml',
    ...
    # hdf5
    if (fc_format is None and fc_ext in hdf5_exts) or\
            (fc_format in hdf5_exts):
        fc_format = 'hdf5'

    # phonopy FORCE_CONSTANTS
    elif (fc_format == 'phonopy' or fc_format is None and fc_ext is None) or\
            (fc_pathname == summary_pathname):
        fc_format = 'phonopy'

    # summary file
    elif (fc_format is None and fc_ext in yaml_exts) or\
            (fc_format in yaml_exts):
        fc_format = 'yaml'
    ...
    if fc_format == 'phonopy':
        if not _check_fc_file_format(fc_pathname):
            raise Exception('Incorrect formatting for {fc_pathname}.')
    ...
    # summary file
    if (born_pathname == summary_pathname) or (born_format in yaml_exts):
        born_format == 'yaml'

    # phonopy BORN file
    elif born_ext is None and (born_format in [None,'','phonopy']):
        born_format = 'phonopy'
    ...
    if born_format == 'phonopy':
        if not _check_born_file(born_pathname):
            raise Exception('Incorrect born formatting for {born_pathname}.')
        elif not _check_born_summary(summary_pathname):
            raise Exception('Born data not present in {summary_pathname}.')
