...
#NOTE this function might be unnecessary assuming extensions will not change.
...
def _extract_ppyaml(ppyaml):
    ...
    #n.p_matrix = np.array(ppyaml['primitive_matrix']) #NOTE what dis?
...
#TODO remove
def _extract_ions_yaml(p_yaml):
    """ DOC
    Retrieve and convert atomic positions from phonopy_disp.yaml or
    phonopy.yaml.
    """

    unit_cell = p_yaml['unit_cell']
    points_list = unit_cell['points']

    n_ions = len(points_list)
    cell_vec = np.array(unit_cell['lattice']) # TODO check dimensions / format

    ion_type = [ion['symbol'] for ion in points_list]
    ion_r = np.array([ion['coordinates'] for ion in points_list])
    ion_mass = np.array([ion['mass'] for ion in points_list])

    #cell_origins = _get_cell_origins(p_yaml)['cell_origins']
    cell_origins = _get_cell_origins(p_yaml).cell_origins #NOTE handling practices?
